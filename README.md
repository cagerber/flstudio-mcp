...
| Running FL Studio under Wine on Linux: requests reach FL but responses/heartbeats never arrive | Some Wine builds' ALSA MIDI driver drops outbound SysEx from inside Wine even though inbound SysEx and short messages work. The controller script already sends every response as a short-message fallback for exactly this case, so update to the latest `device_FLStudioMCP.py` and MCP server; no config change is needed. |
| Wine/Linux: FL's Script output shows `[FLStudioMCP] Ready` and MIDI Settings looks correct, but `fl_ping`/heartbeats still fail | Wine assigns a new numbered ALSA client per enabled MIDI device (`WINE ALSA Output #6`, `#7`, `#3`, ...), so the generic `WINE ALSA Output`/`WINE ALSA Input` substring match can bind the daemon to the wrong port. Use the stable `Midi Through Port-0` client instead: enable it as both Input and Output in FL's MIDI Settings (same Port number, Controller type `FLStudioMCP`), then run the daemon via `scripts/run_daemon_linux.sh` (generated by `install_linux.sh`), which pins `FLSTUDIO_MCP_PORT_TO_FL`/`FLSTUDIO_MCP_PORT_FROM_FL` to `Midi Through Port-0`. |
